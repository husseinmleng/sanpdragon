...
class SDK:
    ...
    def model(self):
        return self.__class__.ROOT / 'examples' / 'Models' / 'inception_v3'
...
def convert_to_dlc(pb_filename, model_dir, tensorflow_dir, runtime, udo, htp_soc):
    ...
    if udo:
        cmd.append('--udo')
        if htp_soc=="sm8650":
            cmd.append(os.path.join(UDO.SNPE_UDO_PATH, 'config', UDO.INCEPTION_V3_UDO_PLUGIN_HTP_V75))
        elif htp_soc=="sm8550":
...
def setup_assets(inception_v3_data_dir, download, runtime, udo, htp_soc):
    ...
    model_dir = SDK.ROOT / 'examples' / 'Models' / 'inception_v3'
